...
const TaskForm: React.FC<TaskFormProps> = ({
  ...
  useEffect(() => {
    if (projectId) {
      const foundProject = projects?.documents.find(
        ({ $id }) => projectId === $id,
      ) as Models.Document | undefined;

      if (foundProject) {
        setProjectName(foundProject.name);
        setProjectColorHex(foundProject.color_hex);
      } else {
        setProjectName('');
        setProjectColorHex('');
      }
  ...
  const handleSubmit = useCallback(() => {
    if (!taskContent) return;
      ...
      console.log('Submitting taskContent:', taskContent);
